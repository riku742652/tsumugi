...
ブラウザ
  ...
  │
  └─ Lambda Function URL      ← API エンドポイント（認証タイプ: NONE）
       │
       └─ Lambda               ← FastAPI + Lambda Web Adapter (Python)
            │                     Authorization: Bearer <JWT> を FastAPI で検証
            ├─ Cognito          ← JWT 発行・ユーザー管理
            │
...
| **Lambda Function URL** | API エンドポイント | API Gateway（今回は不要、コスト増） |
| **Lambda Web Adapter** | FastAPI → Lambda 変換 | カスタムハンドラー（定型コードが増える） |
| **FastAPI** | API ロジック + JWT 検証 | Node.js（Python の方が Cognito JWKS 検証ライブラリが豊富） |
| **DynamoDB** | トランザクション永続化 | RDS（集計は Python 側で処理するため不要）、S3 Select（遅い） |
| **S3** | フロント配信 | — |
...
リクエスト
  ...
  → FastAPI ミドルウェア
      ...
      → 検証成功 → request.state.user_id にセット
