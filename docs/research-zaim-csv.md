...
ブラウザ
  │
  └─ CloudFront ディストリビューション（1つ）
       │
       ├─ /api/*  → Lambda Function URL（オリジン）
       │                └─ Lambda: FastAPI + Lambda Web Adapter
       │                     ├─ X-Origin-Secret ヘッダー検証（CloudFront 以外を拒否）
       │                     ├─ Cognito JWT 検証（python-jose）
       │                     └─ DynamoDB
       │
       └─ /*      → S3 バケット（オリジン）
                       └─ Vite + TypeScript（静的ファイル）
```
...
| **CloudFront** | フロント + API の統合エンドポイント・HTTPS・DDoS 緩和 | API Gateway（コスト増）|
| **Lambda Function URL** | CloudFront のオリジン（直接インターネット公開しない） | — |
| **Lambda Web Adapter** | FastAPI → Lambda 変換 | Mangum（依存関係が増える） |
| **FastAPI** | API ロジック + JWT 検証 | Node.js（Cognito JWKS 検証ライブラリが豊富） |
| **DynamoDB** | トランザクション永続化 | RDS（集計は Python 側で処理するため不要） |
| **S3** | フロント静的ファイル配信（CloudFront OAC 経由のみ） | — |

### セキュリティ設計

**CloudFront → Function URL の保護:**
- CloudFront は毎リクエストにカスタムヘッダー `X-Origin-Secret: <secret>` を付与
- FastAPI ミドルウェアでこのヘッダーを検証し、一致しない場合は 403
- シークレット値は AWS Secrets Manager に保存し、Terraform で管理

**CORS:**
- FastAPI の `allow_origins` を CloudFront ドメインのみに制限

**JWT 検証フロー:**
```
ブラウザ → CloudFront(/api/*) → Lambda Function URL
  → FastAPI: X-Origin-Secret 検証
  → FastAPI: Authorization: Bearer <Cognito ID Token> 検証
      → Cognito JWKS エンドポイントから公開鍵取得（起動時キャッシュ）
